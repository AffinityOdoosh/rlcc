...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    supplier_id = fields.Many2one(comodel_name='res.partner', string='Vendor', required=True,
                                  context={'res_partner_search_mode': 'supplier'})
    item_ids = fields.One2many(comodel_name='purchase.request.line.make.purchase.order.item', inverse_name='wiz_id',
                               string='Items')
    purchase_order_id = fields.Many2one(comodel_name='purchase.order', string='Target Purchase Order',
                                        domain=[('state', '=', 'draft')])
    sync_data_planned = fields.Boolean(string='Match PO Lines by Scheduled Date',
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env['purchase.request.line'].browse(request_line_ids):
            if line.request_id.state not in ['approved']:
                raise UserError(_('Purchase Request %s is not approved or in progress') % line.request_id.name)
            ...
            if picking_type is not False and line_picking_type != picking_type:
                raise UserError(_('You have to select lines from the same Picking Type.'))
            else:
    ...
    def check_group(self, request_lines):
        if len(list(set(request_lines.mapped('request_id.group_id')))) > 1:
            raise UserError(
                _('You cannot create a single purchase order from purchase requests that have different procurement group.'))
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        qty = item.product_uom_id._compute_quantity(item.product_qty, product.uom_po_id or product.uom_id)
    ...
    def _get_order_line_search_domain(self, order, item):
        ...
        if item.line_id.analytic_distribution:
            ...
            order_line_data.append(('analytic_distribution', 'in', analytic_account_ids))
        else:
    ...
    def make_purchase_order(self):
        ...
        for item in self.item_ids:
            ...
            if available_po_lines and not item.keep_description:
                ...
                wizard_product_uom_qty = wizard_uom._compute_quantity(item.product_qty, alloc_uom)
            ...
            date_required = item.line_id.date_required
            po_line.date_planned = user_tz.localize(
                datetime(date_required.year, date_required.month, date_required.day)).astimezone(pytz.utc).replace(
                tzinfo=None)
