...
class PurchaseRequestLine(models.Model):
    ...
    product_uom_id = fields.Many2one(comodel_name='uom.uom', string='Unit of Measure', tracking=True,
    ...
    requested_by = fields.Many2one(comodel_name='res.users', related='request_id.requested_by', string='Requested By',
                                   store=True)
    assigned_to = fields.Many2one(comodel_name='res.users', related='request_id.assigned_to', string='Assigned To',
                                  store=True)
    date = fields.Date(related='request_id.date', store=True)
    description = fields.Text(related='request_id.description', string='Request Description', store=True,
                              readonly=False)
    origin = fields.Char(related='request_id.origin', string='Source Document', store=True)
    date_required = fields.Date(string='Required Date', required=True, tracking=True, default=fields.Date.context_today)
    is_editable = fields.Boolean(compute='_compute_is_editable', readonly=True)
    specifications = fields.Text(string='Specifications')
    request_state = fields.Selection(string='Request Status', related='request_id.state', store=True)
    supplier_id = fields.Many2one(comodel_name='res.partner', string='Preferred Vendor', compute='_compute_supplier_id',
                                  compute_sudo=True, store=True)
    cancelled = fields.Boolean(string='Is Cancelled', readonly=True, default=False, copy=False)

    purchased_qty = fields.Float(string='RFQ/PO Quantity', digits='Product Unit of Measure',
                                 compute='_compute_purchased_qty')
    purchase_lines = fields.Many2many(comodel_name='purchase.order.line',
                                      relation='purchase_request_purchase_order_line_rel',
                                      column1='purchase_request_line_id', column2='purchase_order_line_id',
                                      string='Purchase Order Lines', readonly=True, copy=False)
    purchase_state = fields.Selection(compute='_compute_purchase_state', string='Purchase Status',
                                      selection=lambda self: self.env['purchase.order']._fields[
                                          'state']._description_selection(self.env), store=True)
    move_dest_ids = fields.One2many(comodel_name='stock.move', inverse_name='created_purchase_request_line_id',
                                    string='Downstream Moves')
    orderpoint_id = fields.Many2one(comodel_name='stock.warehouse.orderpoint', string='Reordering Rule')
    ...
    qty_in_progress = fields.Float(string='Quantity In Progress', digits='Product Unit of Measure', readonly=True,
                                   compute='_compute_qty', store=True)
    qty_done = fields.Float(string='Quantity Done', digits='Product Unit of Measure', readonly=True,
                            compute='_compute_qty', store=True)
    qty_cancelled = fields.Float(string='Quantity Cancelled', digits='Product Unit of Measure', readonly=True,
                                 compute='_compute_qty_cancelled', store=True)
    qty_to_buy = fields.Boolean(compute='_compute_qty_to_buy', string='Pending Quantity Available to Purchase',
                                store=True)
    pending_qty_to_receive = fields.Float(compute='_compute_qty_to_buy', digits='Product Unit of Measure', copy=False,
                                          string='Pending Quantity to Receive', store=True)
    ...
    def _compute_qty(self):
        for request in self:
            done_qty = sum(request.purchase_request_allocation_ids.mapped('allocated_product_qty'))
            open_qty = sum(request.purchase_request_allocation_ids.mapped('open_product_qty'))
        ...
        for request in self:
            ...
            if request.product_uom_id:
                request.qty_cancelled = (
                    max(0, request.product_id.uom_id._compute_quantity(qty_cancelled, request.product_uom_id))
        ...
        for rec in self:
            ...
            for line in rec.purchase_lines.filtered(lambda x: x.state != 'cancel'):
                if rec.product_uom_id and line.product_uom != rec.product_uom_id:
                    rec.purchased_qty += line.product_uom._compute_quantity(line.product_qty, rec.product_uom_id)
                else:
    ...
    def _get_supplier_min_qty(self, product, partner_id=False):
        seller_min_qty = 0.0
        if partner_id:
            seller = product.seller_ids.filtered(lambda r: r.partner_id == partner_id).sorted(key=lambda r: r.min_qty)
        else:
    ...
    def _calc_new_qty(self, request_line, po_line=None, new_pr_line=False):
        ...
        if not po_line.order_id.dest_address_id:
            supplierinfo_min_qty = self._get_supplier_min_qty(po_line.product_id, po_line.order_id.partner_id)
    ...
    def unlink(self):
        if self.mapped('purchase_lines'):
            raise UserError(_('You cannot delete a record that is linked to purchase lines.'))
        for line in self:
            if not line._can_be_deleted():
                raise UserError(
                    _('You can only delete a purchase request line if the purchase request is in draft state.'))
        return super().unlink()

    def action_show_details(self):
        self.ensure_one()
        view = self.env.ref('affinity_purchase_request.view_purchase_request_line_details')
        return {
            'name': _('Line Details'),
