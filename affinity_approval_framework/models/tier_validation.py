...
class TierValidation(models.AbstractModel):
    ...
    review_ids = fields.One2many(comodel_name='tier.review', inverse_name='res_id', string='Validations',
                                 domain=lambda self: [('model', '=', self._name)], auto_join=True)
    validated = fields.Boolean(compute='_compute_validated_rejected', search='_search_validated')
    ...
    rejected = fields.Boolean(compute='_compute_validated_rejected', search='_search_rejected')
    rejected_message = fields.Html(compute='_compute_rejected_message')
    validation_status = fields.Selection(selection=[
        ('no', 'Without validation'),
        ('waiting', 'Waiting'),
        ('pending', 'Pending'),
        ('rejected', 'Rejected'),
        ('validated', 'Validated'),
    ], default='no', compute='_compute_validation_status', store=True)
    reviewer_ids = fields.Many2many(string='Reviewers', comodel_name='res.users', compute='_compute_reviewer_ids',
                                    search='_search_reviewer_ids')
    can_review = fields.Boolean(compute='_compute_can_review', search='_search_can_review')
    ...
    def _compute_has_comment(self):
        for rec in self:
            has_comment = rec.review_ids.filtered(
                lambda r: r.status in ('waiting', 'pending') and self.env.user in r.reviewer_ids).mapped('has_comment')
            rec.has_comment = True in has_comment

    def _get_sequences_to_approve(self, user):
        all_reviews = self.review_ids.filtered(lambda r: r.status in ('waiting', 'pending'))
    ...
    def _compute_reviewer_ids(self):
        for rec in self:
            rec.reviewer_ids = rec.review_ids.filtered(lambda r: r.status in ('waiting', 'pending')).mapped(
                'reviewer_ids')
    ...
    def _get_requested_notification_subtype(self):
        return 'affinity_approval_framework.mt_tier_validation_requested'

    def _get_accepted_notification_subtype(self):
        return 'affinity_approval_framework.mt_tier_validation_accepted'

    def _get_rejected_notification_subtype(self):
        return 'affinity_approval_framework.mt_tier_validation_rejected'

    def _get_restarted_notification_subtype(self):
        return 'affinity_approval_framework.mt_tier_validation_restarted'
    ...
    def _add_comment(self, validate_reject, reviews):
        wizard = self.env.ref('affinity_approval_framework.view_comment_wizard')
        return {
    ...
    def _add_tier_validation_buttons(self, node, params):
        str_element = self.env['ir.qweb']._render(
            'affinity_approval_framework.tier_validation_buttons', params
        )
    ...
    def _add_tier_validation_label(self, node, params):
        str_element = self.env['ir.qweb']._render(
            'affinity_approval_framework.tier_validation_label', params
        )
        new_node = etree.fromstring(str_element)
        return new_node
    ...
    def _add_tier_validation_reviews(self, node, params):
        str_element = self.env['ir.qweb']._render(
            'affinity_approval_framework.tier_validation_reviews', params
        )
        new_node = etree.fromstring(str_element)
        return new_node
