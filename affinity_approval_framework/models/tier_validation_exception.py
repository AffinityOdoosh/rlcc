...
class TierValidationException(models.Model):
    ...
    name = fields.Char(required=True, default='New Tier Validation Exception')
    ...
    field_ids = fields.Many2many(comodel_name='ir.model.fields', string='Fields',
                                 domain="[('id', 'in', valid_model_field_ids)]", required=True)
    valid_model_field_ids = fields.One2many(comodel_name='ir.model.fields', compute='_compute_valid_model_field_ids')
    company_id = fields.Many2one(comodel_name='res.company', string='Company', default=lambda self: self.env.company)
    allowed_to_write_under_validation = fields.Boolean(string='Write under Validation', default=True)
    allowed_to_write_after_validation = fields.Boolean(string='Write after Validation', default=True)
    ...
    def _compute_valid_model_field_ids(self):
        ...
        valid_model_fields = dict(
            self.env['ir.model.fields'].sudo()._read_group(domain=[
                ('model', 'in', model_names),
                ('name', 'not in', BASE_EXCEPTION_FIELDS),
            ], groupby=['model'], aggregates=['id:array_agg']))

        for record in self:
            record.valid_model_field_ids = valid_model_fields.get(record.model_name, [])

    @api.constrains('allowed_to_write_under_validation', 'allowed_to_write_after_validation')
    def _check_allowed_to_write(self):
        if (not self.allowed_to_write_under_validation and not self.allowed_to_write_after_validation):
