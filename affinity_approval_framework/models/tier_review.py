...
class TierReview(models.Model):
    ...
    res_id = fields.Many2oneReference(string='Related Document ID', index=True, model_field='model')
    ...
    reviewed_formated_date = fields.Char(string='Validation Formated Date', compute='_compute_reviewed_formated_date')
    has_comment = fields.Boolean(related='definition_id.has_comment')
    comment = fields.Char(string='Comments')
    can_review = fields.Boolean(compute='_compute_can_review', store=True,
                                help='''Can review will be marked if the review is pending and the  approve sequence has been achieved''')
    ...
    def _get_reminder_notification_subtype(self):
        return 'affinity_approval_framework.mt_tier_validation_reminder'

    def _get_reminder_activity_type(self):
        return 'affinity_approval_framework.mail_act_tier_validation_reminder'
    ...
    def _notify_review_reminder(self, record):
        record.message_post(subtype_xmlid=self._get_reminder_notification_subtype(),
                            body=self._notify_review_reminder_body())

    def _schedule_review_reminder_activity(self, record):
        record.activity_schedule(act_type_xmlid=self._get_reminder_activity_type(),
                                 note=self._notify_review_reminder_body(), user_id=self.reviewer_ids.id)
